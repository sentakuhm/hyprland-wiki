...
Full example for hypridel to work with hyprlock:
...
listener {
    timeout = 300                           # 5min
    on-timeout = hyprlock                   # lock screen after timeout has passed
    on-resume = notify-send "Welcome back!" # command to run when activity is detected after timeout has fired.
}

listener {
    timeout = 380                           # 5.5min.
    on-timeout = hyprctl dispatch dpms off  # sleep monitor after timeout has fired.
    on-resume = hyprctl dispatch dpms on    # monitor on when activity is detected after timeout has fired.
}

listener {
    timeout = 1800                          # 30min.
    on-timeout = systemctl suspend          # suspend pc after timeout has passed.
}
```
